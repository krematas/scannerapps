...
if __name__ == '__main__':
    ...
    if opt.save:
        ...
        for j in tqdm(range(1, n_frames), ncols=50):
            ...
            if opt.visualize:
                if j == n_frames-1:
                    ...
                    rgb = rgb * (1 - canvas)[:, :, None] + np.dstack((canvas * 255, np.zeros_like(canvas),
        ...
        for j in range(len(indeces)-1):
            ...
            angles_s = utils.get_angle_from_rotation(CAMERAS_R[start])
            angles_t = utils.get_angle_from_rotation(CAMERAS_R[end])
